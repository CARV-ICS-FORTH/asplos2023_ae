...
parser.add_option("-t", "--legendFile", metavar="PATH", dest="legFile",
                  help="Legend File")
...
for i in range(len(execution)):
    ...
    p1 = ax.bar(tmp_index, other[i], bar_width, color=config.monochrom[10],
                edgecolor=config.edgecolor, zorder=2,
                linewidth=config.edgewidth,
                label="Compute" if i == 0 else None)

    p2 = ax.bar(tmp_index, serdes[i], bar_width, bottom=other[i],
                color=config.monochrom[2], edgecolor=config.edgecolor,
                linewidth=config.edgewidth,
                zorder=2, label="S/D + I/O" if i == 0 else None)

    tmp2 = [x+y for x, y in zip(serdes[i], other[i])]

    p3 = ax.bar(tmp_index, minor[i], bar_width, bottom=tmp2,
                color=config.monochrom[0], edgecolor=config.edgecolor,
                linewidth=config.edgewidth, zorder=2, label="Minor GC"
                if i == 0 else None)

    tmp = [x+y for x, y in zip(tmp2, minor[i])]

    p4 = ax.bar(tmp_index, major[i], bar_width, bottom=tmp,
...
# Export seperate legend
figlegend = pylab.figure(figsize=config.thirdfigsize)

legend = figlegend.legend((p1, p2, p3, p4),
                          ('Other', 'S/D + I/O', 'Minor GC', 'Major GC'),
                          'center', ncol=4, fontsize=config.fontsize)

legend.get_frame().set_linewidth(0.0)

# Save legend
figlegend.savefig('%s' % options.legFile, bbox_inches='tight', dpi=900)
